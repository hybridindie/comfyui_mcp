...
14. **Tests must call actual tool functions.** Test tools by calling the
    functions returned from `register_*_tools()`, module-level decorated
    tool functions, or by using the tool registration dict. Never access
    `_tool_manager` or other private SDK attributes.
15. **Tests must test this project, not libraries.** Don't test that pydantic
    ...
    audit logs are written (by the tool OR by `AuditMiddleware`), correct API
    calls are made.
...
19. **Security-invariant tests must cover both enforcement paths.** When a
    cross-cutting concern (rate limiting, audit logging) migrates from
    in-tool calls to middleware, update `test_security_invariants.py` to
    assert the middleware-based enforcement path for the migrated tools. The
    invariant is "the concern is enforced for every tool call," not "the call
    appears in the tool body."
