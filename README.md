...
| Layer | What it does |
|-------|-------------|
| **Workflow Inspector** | Parses every workflow before execution, extracts node types, flags dangerous patterns (`eval`, `exec`, `__import__`, `subprocess`). First-party ComfyUI cloud API nodes (Kling, Veo, OpenAI, Gemini, Recraft, etc. — 109 nodes across 16 vendors) are flagged by default for network egress + cost. Configurable audit-only or enforcement mode. In enforce mode, dangerous-node and suspicious-input warnings **elicit the user** for confirmation before submission. |
