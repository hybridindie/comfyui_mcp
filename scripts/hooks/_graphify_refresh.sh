#!/usr/bin/env bash
# Shared by the post-commit and post-merge hooks. Refreshes the graphify graph
# STRUCTURE when the latest commit/merge touched MCP source.
#
...
# — run `scripts/graphify.sh label .` to refresh names. See
...
printf '%s\n' "$changed" | grep -qE '^src/comfyui_mcp/.*\.py$' || exit 0
