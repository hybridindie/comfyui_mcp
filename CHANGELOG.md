...
- **Server-side node replacement warnings** — the workflow inspector now
  ...
  This closes the gap where the inspector vets the pre-submission graph
  but the server replaces nodes after inspection — what executes may
  differ from what was vetted. Warnings are emitted in both
  `validate_workflow` and `_submit_workflow` (generation tools). The
  replacement check recurses into subgraphs (#111).
