...
@pytest.fixture
def inspector() -> WorkflowInspector:
...
class TestStructuralValidation:
    ...
    async def test_valid_workflow_passes(
        self, client: ComfyUIClient, inspector: WorkflowInspector
    ) -> None:
...
class TestServerValidation:
    @respx.mock
    async def test_missing_node_type_is_error(
        self, client: ComfyUIClient, inspector: WorkflowInspector
    ) -> None:
        respx.get("http://test:8188/object_info").mock(
            return_value=httpx.Response(
    ...
    @respx.mock
    async def test_missing_model_is_warning(
        self, client: ComfyUIClient, inspector: WorkflowInspector
    ) -> None:
        respx.get("http://test:8188/object_info").mock(
            return_value=httpx.Response(
    ...
    @respx.mock
    async def test_server_unreachable_adds_warning(
        self, client: ComfyUIClient, inspector: WorkflowInspector
    ) -> None:
        respx.get("http://test:8188/object_info").mock(side_effect=httpx.ConnectError("offline"))
        wf = _valid_workflow()
...
class TestSecurityValidation:
    @respx.mock
    async def test_dangerous_node_adds_warning(
        self, client: ComfyUIClient, inspector: WorkflowInspector
    ) -> None:
        respx.get("http://test:8188/object_info").mock(side_effect=httpx.ConnectError("offline"))
        wf: dict[str, Any] = {"1": {"class_type": "EvalNode", "inputs": {}}}
        result = await validate_workflow(wf, client, inspector)
        assert any("Dangerous" in w for w in result["warnings"])

    @respx.mock
    async def test_enforce_mode_blocks(self, client: ComfyUIClient) -> None:
        respx.get("http://test:8188/object_info").mock(side_effect=httpx.ConnectError("offline"))
