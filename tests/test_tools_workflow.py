...
@pytest.fixture
def components(tmp_path: Any) -> tuple[ComfyUIClient, AuditLogger, RateLimiter, WorkflowInspector]:
...
class TestCreateWorkflow:
    async def test_creates_txt2img(
        self, components: tuple[ComfyUIClient, AuditLogger, RateLimiter, WorkflowInspector]
    ) -> None:
    ...
    async def test_creates_with_params(
        self, components: tuple[ComfyUIClient, AuditLogger, RateLimiter, WorkflowInspector]
    ) -> None:
        client, audit, limiter, inspector = components
        mcp = FastMCP("test")
...
class TestModifyWorkflow:
    ...
    async def test_invalid_workflow_json_raises(
        self, components: tuple[ComfyUIClient, AuditLogger, RateLimiter, WorkflowInspector]
    ) -> None:
        client, audit, limiter, inspector = components
        mcp = FastMCP("test")
    ...
    async def test_invalid_operations_json_raises(
        self, components: tuple[ComfyUIClient, AuditLogger, RateLimiter, WorkflowInspector]
    ) -> None:
        client, audit, limiter, inspector = components
        mcp = FastMCP("test")
...
class TestIntegration:
    @respx.mock
    async def test_create_modify_validate_roundtrip(
        self, components: tuple[ComfyUIClient, AuditLogger, RateLimiter, WorkflowInspector]
    ) -> None:
        client, audit, limiter, inspector = components
