...
| Layer | What it does |
|-------|-------------|
| **Workflow Inspector** | Parses every workflow before execution, extracts node types, flags dangerous patterns (`eval`, `exec`, `__import__`, `subprocess`). Recurses into subgraph nodes so nested dangerous nodes can't evade inspection. First-party ComfyUI cloud API nodes (Kling, Veo, OpenAI, Gemini, Recraft, etc. — 135 nodes across 16 vendors) are flagged by default for network egress + cost. Warns when a submitted `class_type` has a server-side replacement (`/node_replacements`) — what executes may differ from what was vetted. Configurable audit-only or enforcement mode. In enforce mode, dangerous-node and suspicious-input warnings **elicit the user** for confirmation before submission. |
